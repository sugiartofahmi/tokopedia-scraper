...
export const Tokopedia = async (page, pages) => {
  ...
  for (let x = 1; x <= pages; x++) {
    await page.goto(
      `https://www.tokopedia.com/search?navsource=&ob=${process.env.OB}&pmax=${process.env.PMAX}&pmin=${process.env.PMIN}&shop_tier=1%233%231%232&q=${process.env.SEARCH_QUERY}&page=${x}`
    );
